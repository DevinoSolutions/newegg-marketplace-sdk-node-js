...
| `newegg_orders_list`              | `{ orderNumbers?, sellerOrderNumbers?, status?, type?, includeDownloaded?, premierOrder?, dateFrom?, dateTo?, countryCode?, page?, pageSize? (≤100) }` | Read-only. Paged Get Order Information search; returns fully-detailed orders (line items, packages, customer ship-to).                                                    |
| `newegg_orders_get`               | `{ orderNumber: string \| integer }`                                                                                                                   | Read-only. One order's full detail; `found=false` when no such order exists for this seller.                                                                              |
| `newegg_orders_get_status`        | `{ orderNumber: string \| integer }`                                                                                                                   | Read-only. Lightweight order status (status code/name, sales channel, fulfillment); `found=false` when absent.                                                            |
...
- **Order `status`** ∈ `unshipped | partiallyShipped | shipped | invoiced | voided | paymentPending`; **`type`** ∈ `all | sbn | sbs | multiChannel | nws`; **`premierOrder`** ∈ `all | premierOnly | noPremier`. `dateFrom`/`dateTo` are ISO 8601 and are converted to Newegg Pacific time.
